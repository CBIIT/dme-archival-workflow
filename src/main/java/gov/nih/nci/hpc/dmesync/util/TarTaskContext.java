...
public final class TarTaskContext {
    ...
    /** Store the list of broken symlink files that were excluded from the tar archive when ignore symlink property is enabled. */
    public static void setExcludedFiles(List<File> files) {
        excludedFiles.set(files);
    }
    /**
     * Returns the list of broken symlink files that were excluded from the tar archive when ignore symlink property is enabled, or {@code null} if
