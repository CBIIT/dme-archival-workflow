...
public class POBCCDIPathMetadataProcessorImpl extends AbstractPathMetadataProcessor
	...
	public String getArchivePath(StatusInfo object) throws DmeSyncMappingException {
		...
		if (StringUtils.equalsIgnoreCase("Fastq", subCollectionName)) {
			archivePath = destinationBaseDir + "/Lab_" + getPiCollectionName(object, metadataFilePathKey) + "/Project_"
					+ getProjectCollectionName(object, metadataFilePathKey) + "/Fastq/" + getSeqCollectionName(object)
					+ "/" + fileName;
		} else {
			
			if(hasMetricsFile(object)) {
				// if combined_summary_metrics.xls then directly upload under analysis folder
				archivePath = destinationBaseDir + "/Lab_" + getPiCollectionName(object, metadataFilePathKey) + "/Project_"
						+ getProjectCollectionName(object, metadataFilePathKey) + "" + "/Analysis/" + fileName;
	...
	public HpcDataObjectRegistrationRequestDTO getMetaDataJson(StatusInfo object)
		...
		try {
			...
			pathEntriesPI.getPathMetadataEntries().add(
					createPathEntry("data_owner", labCollectionName));
			pathEntriesPI.getPathMetadataEntries().add(
			...
			pathEntriesProject.getPathMetadataEntries().add(
					createPathEntry("key_collaborator", getAttrValueWithKey(metadataFilePathKey, "key_collaborator")));
			...
			pathEntriesProject.getPathMetadataEntries()
					.add(createPathEntry("Collaborators", getAttrValueWithKey(metadataFilePathKey, "Collaborators")));
			...
			if (StringUtils.equalsIgnoreCase(subCollectionName, "Fastq")) {
				...
				pathEntriesSeq.getPathMetadataEntries()
						.add(createPathEntry("analyte_type", getAttrValueWithKey(metadataFilePathKey, "data_type")));
				pathEntriesSeq.getPathMetadataEntries()
				...
				if (StringUtils.equalsIgnoreCase(analysisCollectionName, PRIMARY_ANALYSIS_INPUT_NAME)) {
					...
					if (geonomeType != null && geonomeType.startsWith("GRCh")) {
						String geonomeCollectionPath = analysisSubCollectionPath + "/"+geonomeType;
						...
						pathEntriesGeonome.getPathMetadataEntries().add(createPathEntry("mapped_genome",
	...
	public String getPathForMetadata(StatusInfo object) {
		...
		// The data is in helix and below logic discards the mnt path /vf/users in originalfilePath.
	...
	private String getPathFromParent(StatusInfo object, String parentName) {
		 ...
		 * /data/CCRCCDI/dme_test/CS035485_Shern_Zhang_3PGEX/PrimaryAnalysisInputFiles /tarfile
		 * parent name: dme_test then parent path:
		 * /data/CCRCCDI/dme_test/CS035485_Shern_Zhang_3PGEX*/
	...
	private boolean hasMetricsFile(StatusInfo object) {
		boolean hasMetricsFile= StringUtils.equalsIgnoreCase((Paths.get(object.getSourceFilePath()).getParent().getFileName().toString()),"ExtractedData");
		logger.info("Id this combined_metrics excel file upload: {}", hasMetricsFile);
		return hasMetricsFile;
