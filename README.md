...
* `dmesync.framework.metadata.validation.enabled=[true|false]`
  * If `true`, enables framework-level metadata validation against rules fetched from DME API.
  * Default: `false`
* `dmesync.framework.metadata.validation.validateObjectMetadata=[true|false]`
  * If `true`, validates object metadata in addition to collection metadata.
  * Default: `false`
* `dmesync.framework.rules.path=<API endpoint path>`
  * DME API endpoint path for fetching framework validation rules.
  * Default: `/metadata/frameworkRules`
...
The DME archival workflow now supports framework-level metadata validation to ensure that all generated metadata complies with the validation rules defined in the DME system. This feature validates metadata **before** it is persisted or uploaded, catching errors early in the workflow.

### How It Works

1. **Rule Retrieval**: When enabled, the workflow fetches validation rules from the DME API endpoint (configurable via `dmesync.framework.rules.path`).
2. **Rule Caching**: Rules are cached using Spring Cache to minimize API calls and improve performance.
3. **Validation**: After metadata is generated by the DOC-specific processor but before it's saved, the framework validates:
   - **Collection Metadata**: Validates metadata for all parent collections
   - **Object Metadata** (optional): Validates data object metadata if enabled
4. **Error Reporting**: If validation fails, a comprehensive error message is generated listing all violations, and the workflow stops processing that file.

### Configuration

Enable validation in `application.properties`:

```properties
# Enable framework metadata validation
dmesync.framework.metadata.validation.enabled=true

# Optionally validate object metadata (in addition to collection metadata)
dmesync.framework.metadata.validation.validateObjectMetadata=false

# DME API endpoint for framework rules (default shown)
dmesync.framework.rules.path=/metadata/frameworkRules
```

### Validation Rules

The framework supports the following validation rule types:

- **Mandatory Attributes**: Ensures required attributes are present and non-blank
- **Valid Values**: Restricts attribute values to a predefined list
- **Collection Type Filtering**: Applies rules only to specific collection types (e.g., `PI_Lab`, `Project`)
- **Rule Enablement**: Rules can be disabled without removing them from the system
...
```json
{
  ...
  "mandatory": true,
  "collectionTypes": ["PI_Lab"],
  "ruleEnabled": true,
  "description": "First and Last name of the data owner"
}
```

This rule enforces that all `PI_Lab` collections must have a non-blank `data_owner` attribute.

### Error Message Example

When validation fails, you'll see a clear error message like:

```
Framework metadata validation failed for doc 'cmm' (file: /data/CMM_CryoEM/sample.tar):
  - RuleSet[63fdccdd-64b8-477f-9e5c-450c4dccf748] Path[/CCR_CMM_Archive/PI_001]: Mandatory attribute 'data_owner' is missing or blank
  - RuleSet[63fdccdd-64b8-477f-9e5c-450c4dccf748] Path[/CCR_CMM_Archive/Project_HIV]: Attribute 'collection_type' has invalid value 'InvalidType'. Valid values: [Project, PI_Lab, Sample]
```

### Benefits

- **Early Error Detection**: Catches metadata issues before upload
- **Consistent Validation**: Same rules apply to all DOC processors
- **Non-Invasive**: Disabled by default; no impact on existing workflows
- **Centralized Rule Management**: Rules are managed in DME, not in code
- **Performance**: Cached rules minimize API overhead

### Notes

- Framework validation is **disabled by default** to maintain backward compatibility
- Rules are fetched from DME API and cached per DOC
- The `defaultValue` field in rules is **not** used for auto-filling; missing mandatory attributes will cause validation failure
- Validation only checks the generated metadata structure; it does not validate file contents or checksums
